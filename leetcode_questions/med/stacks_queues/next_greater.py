...
def test_next_greater():
    """pytest next_greater.py"""
    assert next_greater([4, 1, 2], [1, 3, 4, 2]) == [-1, 3, -1]
    assert next_greater([2, 4], [1, 2, 3, 4]) == [3, -1]


def next_greater(nums1: list[int], nums2: list[int]) -> list[int]:
    """
    [4, 1, 2]
    ...
    map = {
        ...
        # handle left in stack
        4: -1

        # handle rest in nums1
        2: -1
    }
    res = -1 3 -1
    """
    ...
    # iterate on nums2 to create hashmap
    for i in range(1, len(nums2)):
        ...
        print(f"i {i} stack {stack} hash {hash}")

    # if anything left, add to hash
    # these have not found a next greater
    for e in stack:
        hash[e] = -1
