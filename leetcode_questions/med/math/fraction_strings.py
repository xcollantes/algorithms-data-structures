...
def test_fraction_strings_simple():
    ...
    assert fraction_strings(["2/6+2/6", "7/10+13/10"]) == ["2/3", "2/1"]
    assert fraction_strings(["1/8+1/9"]) == ["17/72"]


def test_fraction_strings_negatives():
    """pytest fraction_strings.py"""
    assert fraction_strings("-1/2+1/2") == "0/1"
    assert fraction_strings("-1/2+1/2+1/3") == "1/3"
    assert fraction_strings("1/3-1/2") == "-1/6"


def fraction_strings(input: str) -> str:
    """
    n1
    /
    d1
